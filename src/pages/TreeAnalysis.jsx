...
import { analyzeTree, getTreeHistory, mockTreeAnalysis, mockTreeHistory, isMockMode, BudgetError } from '../services/weatherApi'
...
export default function TreeAnalysis({ farm, onBack, onBudgetError }) {
  ...
  async function runAnalysis() {
    ...
    try {
      let data
      if (isMockMode()) {
        const b = checkBudget(); if (!b.allowed) { onBudgetError(b.reason); return }
        consumeApiCall()
        await new Promise(r => setTimeout(r, 2200))
        data = mockTreeAnalysis()
        data.overlay_url = preview
      } else {
        try {
          const fd = new FormData()
          fd.append('image', file)
          if (farm) { fd.append('lat', farm.lat); fd.append('lng', farm.lng) }
          data = await analyzeTree(fd)
          // normalise overlay field
          data.overlay_url = data.overlay_url ?? data.annotated_url ?? data.image_url ?? preview
          data.tree_count  = data.tree_count  ?? data.trees ?? 0
          data.canopy      = data.canopy ?? { healthy: data.healthy_pct ?? 70, needs_care: data.needs_care_pct ?? 20, needs_replacement: data.needs_replacement_pct ?? 10 }
        } catch (e) {
              ...
              {history.map(h => (
                <div key={h.id} className="flex items-center gap-4 bg-white/5 border border-white/8 rounded-2xl p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-white/40 text-xs mt-0.5">{new Date(h.analyzed_at).toLocaleDateString('en-KE', { year:'numeric', month:'short', day:'numeric', hour:'2-digit', minute:'2-digit' })}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-emerald-400 text-xs font-bold">{h.canopy.healthy}% healthy</p>
                    <p className="text-amber-400 text-xs">{h.canopy.needs_care}% needs care</p>
                  </div>
                </div>
              ))}
            </div>
