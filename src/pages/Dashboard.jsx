...
const CONDITION_ICON = {
  ...
  'Heavy Rain': '🌧️', 'Thunderstorm': '⛈️', 'Foggy': '🌫️',
}
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
      ...
      <div className="flex justify-between mt-2">
        <span className="text-white/40 text-[10px]">{limit - used} calls remaining</span>
    ...
    <div
      ...
      <div className="relative p-6">
        ...
        <div className="mt-5 pt-4 border-t border-white/8 flex items-center justify-between">
          ...
          <span className="text-white/30 text-[10px] flex items-center gap-1 group-hover:text-[#a8d66b] transition-colors">
            View details →
          </span>
        </div>
      </div>
...
function BudgetBanner({ reason, onDismiss }) {
  return (
    <div className="fixed top-5 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-red-950 border border-red-500/40 rounded-2xl px-5 py-3.5 shadow-2xl"
      ...
      <button onClick={onDismiss} className="text-red-400/60 hover:text-red-300 ml-2 text-lg leading-none">✕</button>
    </div>
  )
}
...
export default function Dashboard({ onLogout }) {
  ...
  // Mock weather fetch — enforces budget
  async function fetchWeather(farm) {
    ...
    return {
      temperature: 14 + Math.random() * 18,
      condition: ['Partly Cloudy', 'Sunny', 'Light Rain', 'Overcast', 'Foggy'][Math.floor(Math.random() * 5)],
  ...
  function handleLogout() { signOut(); onLogout() }
    ...
    <div className="min-h-screen text-white" style={{ background: 'linear-gradient(160deg, #071510 0%, #0d2318 40%, #071510 100%)' }}>
      ...
      <nav className="sticky top-0 z-40 border-b border-white/8"
        ...
        <div className="max-w-7xl mx-auto px-6 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-2 font-black text-lg">
            ...
            <span className="text-white">Farm<span className="text-[#a8d66b]">Pulse</span></span>
          </div>

          <div className="hidden md:flex items-center gap-6 text-sm">
            <span className="text-[#a8d66b] font-semibold border-b border-[#a8d66b] pb-0.5">Dashboard</span>
            {['Farm Detail', 'Tree Analysis', 'Alert Feed'].map(p => (
              <span key={p} className="text-white/40 hover:text-white/70 cursor-pointer transition-colors">{p}</span>
            ))}
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={handleLogout}
              className="text-white/40 hover:text-white/80 text-xs transition-colors border border-white/10 hover:border-white/20 px-3 py-1.5 rounded-lg">
              Sign out
            </button>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8" style={{ animation: 'fadeSlideUp 0.5s ease-out' }}>
          <p className="text-[#a8d66b] text-xs font-bold tracking-widest uppercase mb-1">Good morning</p>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
            ...
            {farms.length === 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 text-center border border-dashed border-white/10 rounded-3xl"
                ...
                <p className="text-white/40 text-sm mb-8 max-w-xs leading-relaxed">
                  Add your first farm to start monitoring live weather, crop health, and risk alerts.
                </p>
                <button onClick={() => setShowModal(true)}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                {farms.map((farm, i) => <FarmCard key={farm.id} farm={farm} index={i} />)}
              </div>
            )}
          </div>
          ...
          <div className="lg:w-72 shrink-0 space-y-4" style={{ animation: 'fadeSlideUp 0.5s ease-out 0.2s both' }}>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
              ...
              {[
                { color: '#10b981', label: 'Safe', desc: 'Conditions are normal' },
                { color: '#f59e0b', label: 'Watch', desc: 'Monitor closely' },
                { color: '#ef4444', label: 'Act Now', desc: 'Immediate action needed' },
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
              ...
              {[
                ...
                '💰 Budget enforced before every call',
              ].map(s => (
      ...
      {showModal && (
        <AddFarmModal onClose={() => setShowModal(false)} onFarmAdded={handleFarmAdded} />
      )}
    </div>
  )
}
